...
def extract_feature(model,dataloaders):
    ...
    for data in dataloaders:
        ...
        count += n
        print(count)
        ff = torch.FloatTensor(n,opt.featurereid).zero_()
        for i in range(2):
            if(i==1):
                img = fliplr(img)
            input_img = Variable(img.cuda())
            outputs= model(input_img,0)
            feature = outputs[0]
...
if __name__ == '__main__':
    print('-------test-----------')
    model_structure = ReIDNet(class_num=len(class_names)+1,testing=opt.testing,usinghpa=True,hpamodel='densenet',hpaepoch='last',featuresize=opt.featuresize)
    input = Variable(torch.FloatTensor(8, 3, 384, 128))
    output = model_structure(input,epoch = 0)
    for i in output:
        print(i.shape)
