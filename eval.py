...
def build_network(snapshot, backend):
    ...
    # net = models[backend]()
    net = HPANet(n_classes=20, sizes=(1, 2, 3, 6), modelname=args.backend, pretrained=True, usinglargefeature=True)
...
def main():
    ...
    with torch.no_grad():
        for index, (img, gt) in enumerate(val_loader):
            pred_seg, pred_cls,pred_call = net(img.cuda())
            pred_seg = pred_seg[0]
